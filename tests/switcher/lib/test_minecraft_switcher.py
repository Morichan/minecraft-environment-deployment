...
class TestMinecraftSwitcher:
    def create_cfn_parameters(self, parameters, is_reused=None):
        return [({
            'ParameterKey': k,
            'ParameterValue': v
        } | ({'UsePreviousValue': is_reused} if is_reused is not None else {})) for k, v in parameters.items()]

    def create_cfn_template_body(self, parameters):
        return {
    ...
    def create_cfn_stack(self, stack_name, parameters={}):
        ...
        with mock_s3():
            ...
            cfn.create_stack(
                ...
                TemplateBody=json.dumps(self.create_cfn_template_body(parameters)),
                Parameters=self.create_cfn_parameters(parameters)
            )
    ...
    def test_get_cloudformation_one_parameter(self):
        ...
        self.create_cfn_stack(stack_name, stack_params)
        obj = MinecraftSwitcher(stack_name)
        expected = self.create_cfn_parameters(stack_params)
    ...
    def test_not_get_cloudformation_if_try_to_search_different_stack_name(self):
        stack_params = {'SampleKey': 'SampleValue', 'ExampleKey': 'ExampleValue'}
        self.create_cfn_stack('SampleStack', stack_params)
    ...
    def test_update_cloudformation_to_override_all_parameters(self):
        ...
        self.create_cfn_stack(stack_name, stack_params)
        obj = MinecraftSwitcher(stack_name)
        expected = self.create_cfn_parameters(overrode_params)
    ...
    def test_update_cloudformation_to_override_any_parameters(self):
        ...
        self.create_cfn_stack(stack_name, stack_params)
        obj = MinecraftSwitcher(stack_name)
        expected = self.create_cfn_parameters(overrode_params | ignore_overrode_params)
